...
void 
LteEnbRrc::SetConnectionEstablishedCallback(Callback<void, uint64_t, uint16_t, uint16_t> callback)
{
...
void
LteEnbRrc::SetHandoverJoiningTimeoutCallback(Callback<void, uint16_t> callback)
{
  NS_LOG_FUNCTION (this << &callback);
  m_handoverJoiningTimeoutCallback = callback;
}

void
LteEnbRrc::SetHandoverLeavingTimeoutCallback(Callback<void, uint16_t> callback)
{
  NS_LOG_FUNCTION (this << &callback);
  m_handoverLeavingTimeoutCallback = callback;
}
...
UeManager::RecvRrcConnectionSetupCompleted (LteRrcSap::RrcConnectionSetupCompleted msg)
{
  ...
  switch (m_state)
    ...
    case CONNECTION_SETUP:
      ...
      if (!m_rrc->m_connectionEstablishedCallback.IsNull ())
      	{
      		m_rrc->m_connectionEstablishedCallback (m_imsi, m_rrc->ComponentCarrierToCellId (m_componentCarrierId), m_rnti);
      	}
      break;
...
LteEnbRrc::HandoverJoiningTimeout (uint16_t rnti)
{
  ...
  if (!m_handoverJoiningTimeoutCallback.IsNull())
  	{
  		m_handoverJoiningTimeoutCallback(rnti);
  	}
  RemoveUe (rnti);
}
...
LteEnbRrc::HandoverLeavingTimeout (uint16_t rnti)
{
  ...
  if (!m_handoverLeavingTimeoutCallback.IsNull())
  	{
  		m_handoverLeavingTimeoutCallback(rnti);
  	}
  RemoveUe (rnti);
}
