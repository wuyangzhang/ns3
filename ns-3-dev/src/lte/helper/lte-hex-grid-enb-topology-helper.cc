...
LteHexGridEnbTopologyHelper::LteHexGridEnbTopologyHelper ()
{
  ...
  for (int i = 0; i < 3; i++)
  	{
  		m_dlEarfcnList.push_back(100*(i+1));
  		m_ulEarfcnList.push_back(100*(i+1)+18000);
  	}
}
...
void LteHexGridEnbTopologyHelper::SetDlEarfcnList(std::vector<uint32_t> earfcnList)
{
	m_dlEarfcnList = earfcnList;
}

void LteHexGridEnbTopologyHelper::SetUlEarfcnList(std::vector<uint32_t> earfcnList)
{
	m_ulEarfcnList = earfcnList;
}
...
LteHexGridEnbTopologyHelper::SetPositionAndInstallEnbDevice (NodeContainer c)
{
  ...
  for (uint32_t n = 0; n < c.GetN (); ++n)
	...
	case 2:
      ...
      m_lteHelper->SetEnbDeviceAttribute ("DlEarfcn", UintegerValue(m_dlEarfcnList[n%3]));
      m_lteHelper->SetEnbDeviceAttribute ("UlEarfcn", UintegerValue(m_ulEarfcnList[n%3]));
