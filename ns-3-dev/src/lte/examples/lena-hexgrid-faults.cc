...
NS_LOG_COMPONENT_DEFINE ("LenaHexgridFaults");
          ...
          if (uedev)
            {
              ...
              outFile << "set label \"UE-" << uedev->GetImsi ()
                      << "\" at "<< pos.x << "," << pos.y << " left font \"Helvetica,8\" textcolor rgb \"grey\" front point pt 4 ps 0.3 lc rgb \"grey\" offset 0,0"
                      << std::endl;
            }
        }
    }
  				...
  				for (int j = 0; j < nDevs; j++)
  					{
  						...
  						if (enbdev)
  							{
  								Vector pos = node->GetObject<MobilityModel> ()->GetPosition ();
  								outFile << "set label \"ENB-" << (int)(enbdev->GetCellId () / 3.0)
                      		<< "\" at "<< pos.x << "," << pos.y
													<< " left font \"Helvetica,12\" textcolor rgb \"white\" front  point pt 8 ps 0.3 lc rgb \"white\" offset 0,0"
													<< std::endl;
  							}
  					}
  			}
...
public:
	...
	void
	NotifyHandoverJoiningTimeout (uint16_t rnti)
	{
	  std::cout << "******************************************** Handover Joining Timeout for RNTI " + rnti
	  					<< std::endl;
	}

	void
	NotifyHandoverLeavingTimeout (uint16_t rnti)
	{
		std::cout << "******************************************** Handover Leaving Timeout for RNTI " + rnti
							<< std::endl;
	}
...
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::UdpClient::Interval", TimeValue (MilliSeconds (100.0)));
  Config::SetDefault ("ns3::UdpClient::MaxPackets", UintegerValue (1e9));
  Config::SetDefault ("ns3::LteRlcUm::MaxTxBufferSize", UintegerValue (1e3 * 1024));
...
//  lteHelper->SetPathlossModelAttribute ("ShadowSigmaExtWalls", DoubleValue (0));
  lteHelper->SetPathlossModelAttribute ("ShadowSigmaOutdoor", DoubleValue (8.0));
...
//                                                TimeValue (MilliSeconds (256)));
  ...
  lteHelper->SetEnbAntennaModelAttribute ("Beamwidth",   DoubleValue (70.0));
  lteHelper->SetEnbAntennaModelAttribute ("BeamwidthVert",   DoubleValue (10.0));
  lteHelper->SetEnbAntennaModelAttribute ("MaxAttenuation",     DoubleValue (25.0));
  lteHelper->SetEnbAntennaModelAttribute ("MaxAttenuationVert",     DoubleValue (20.0));
  lteHelper->SetEnbAntennaModelAttribute ("Downtilt",     DoubleValue (9.0));
...
//  for (uint32_t d = 0; d < macroEnbDevs.GetN(); d++)
//  	{
//  		uint32_t dlEarfcn = macroEnbDevs.Get(d)->GetObject<LteEnbNetDevice>()->GetDlEarfcn();
//  		uint32_t ulEarfcn = macroEnbDevs.Get(d)->GetObject<LteEnbNetDevice>()->GetUlEarfcn();
//
//  		std::cout << "Device " << d << " cell " << d/3 << " sector " << d%3
//  							<< " DL-EARFCN==" << dlEarfcn << " DL-freq== " << LteSpectrumValueHelper::GetCarrierFrequency(dlEarfcn)
//  							<< " UL-EARFCN==" << ulEarfcn << " UL-freq== " << LteSpectrumValueHelper::GetCarrierFrequency(ulEarfcn)
//  							<< std::endl;
//  	}

  // Decrease TX power of sector 0 of cell 0
//  Ptr<LteEnbPhy> phy = macroEnbDevs.Get(0)->GetObject<LteEnbNetDevice>()->GetPhy();
//  phy->SetTxPower(10.0);

  // Change antenna tilt of sector 0 cell 1
//  Ptr<Parabolic3DAntennaModel> antenna = macroEnbDevs.Get(3)->GetObject<LteEnbNetDevice>()->GetPhy()->GetDlSpectrumPhy()->GetRxAntenna()->GetObject<Parabolic3DAntennaModel>();
//  antenna->SetDowntilt(0.0);
  ...
  HandoverHandler hoHdlr;
  Config::Connect ("/NodeList/*/DeviceList/*/LteEnbRrc/ConnectionEstablished",
                   MakeCallback (&HandoverHandler::NotifyConnectionEstablishedEnb, &hoHdlr));
  Config::Connect ("/NodeList/*/DeviceList/*/LteUeRrc/ConnectionEstablished",
                   MakeCallback (&HandoverHandler::NotifyConnectionEstablishedUe, &hoHdlr));
  Config::Connect ("/NodeList/*/DeviceList/*/LteEnbRrc/HandoverStart",
                   MakeCallback (&HandoverHandler::NotifyHandoverStartEnb, &hoHdlr));
  Config::Connect ("/NodeList/*/DeviceList/*/LteUeRrc/HandoverStart",
                   MakeCallback (&HandoverHandler::NotifyHandoverStartUe, &hoHdlr));
  Config::Connect ("/NodeList/*/DeviceList/*/LteEnbRrc/HandoverEndOk",
                   MakeCallback (&HandoverHandler::NotifyHandoverEndOkEnb, &hoHdlr));
  Config::Connect ("/NodeList/*/DeviceList/*/LteUeRrc/HandoverEndOk",
                   MakeCallback (&HandoverHandler::NotifyHandoverEndOkUe, &hoHdlr));

  for (uint32_t d; d < macroEnbDevs.GetN(); d++)
  	{
  			Ptr<LteEnbRrc> rrc = macroEnbDevs.Get(d)->GetObject<LteEnbNetDevice>()->GetRrc();
				rrc->SetHandoverJoiningTimeoutCallback(MakeCallback(&HandoverHandler::NotifyHandoverJoiningTimeout, &hoHdlr));
				rrc->SetHandoverLeavingTimeoutCallback(MakeCallback(&HandoverHandler::NotifyHandoverLeavingTimeout, &hoHdlr));
  	}
